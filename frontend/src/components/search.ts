...
function displayOptions() {
  ...
  const nameHTML = `<li><a class="dropdown-item rounded-t bg-white hover:bg-gray-400 py-2 px-4 block whitespace-no-wrap">Name</a></li>`;
  const statusHTML = `<li><a class="dropdown-item rounded-t bg-white hover:bg-gray-400 py-2 px-4 block whitespace-no-wrap">Status</a></li>`;
  const publicHTML = `<li><a class="dropdown-item rounded-t bg-white hover:bg-gray-400 py-2 px-4 block whitespace-no-wrap">Public</a></li>`;
  const privateHTML = `<li><a class="dropdown-item rounded-t bg-white hover:bg-gray-400 py-2 px-4 block whitespace-no-wrap">Private</a></li>`;
...
async function changePage() {
  const searchContext = document.getElementById("selected-option")!.innerHTML;
  if (searchContext == "Private" || searchContext == "Public") {
    const searchInput: SearchInput = {
      context: parseSearchContext(searchContext),
      criteria:
        (document.getElementById("search-input") as HTMLInputElement).value ||
...
function displayMessage(messageContent: MessageContent) {
  const div = document.createElement("div");
